...
$modal_mode = !empty($_GET['modal_mode']) && $_GET['modal_mode'] === '1';
...
// Runs unconditionally — even when items_id === 0 — so that only
// recognised vehicle types reach CalendarService::getEvents().
global $DB;
...
if ($itemtype !== '' && !isset($allowed_itemtypes[$itemtype])) {
    \Toolbox::logInFile('fleetbooking', sprintf(
        '[calendar.php] Itemtype "%s" NOT in allowed list — rejected.',
        $itemtype
    ));
    echo json_encode([]);
    exit;
...
try {
    ...
    $events = $service->getEvents($itemtype, $items_id, $start, $end, $current_request_id, $modal_mode);
    \Toolbox::logInFile('fleetbooking', sprintf(
