...
function plugin_fleetbooking_install_db(bool $is_upgrade = false)
{
    global $DB;
    ...
    $DB->doQuery("CREATE TABLE IF NOT EXISTS `glpi_plugin_fleetbooking_requests` (
        ...
        `policy_accepted_ip`   varchar(45)       DEFAULT NULL,
        `driver_id_type`       varchar(16)       NOT NULL DEFAULT 'cpf',
        `driver_cpf`           varchar(14)       DEFAULT NULL,
        `driver_registration`  varchar(32)       DEFAULT NULL,
        `driver_cnh_number`    varchar(20)       NOT NULL DEFAULT '',
        `driver_cnh_category`  varchar(5)        NOT NULL DEFAULT '',
        `driver_cnh_expiry`    date              DEFAULT NULL,
        `term_document_id`     int unsigned      DEFAULT NULL,
    ...
    $DB->doQuery("CREATE TABLE IF NOT EXISTS `glpi_plugin_fleetbooking_configs` (
        ...
        `term_template_path`            varchar(255)  DEFAULT NULL,
        `date_creation`                 timestamp     NULL DEFAULT CURRENT_TIMESTAMP,
        `date_mod`                      timestamp     NULL DEFAULT CURRENT_TIMESTAMP ON UPDATE CURRENT_TIMESTAMP,
        PRIMARY KEY (`id`),
    ...
    plugin_fleetbooking_create_vehicle_asset($is_upgrade);
 ...
 * @return void
 */
function plugin_fleetbooking_create_vehicle_asset(bool $is_upgrade = false)
{
    global $DB;
    ...
    if (!$DB->tableExists(\Glpi\Asset\AssetDefinition::getTable())) {
        return;
    }
    ...
    if ($existing_row = $existing->current()) {
        ...
        $DB->insert(\Glpi\Asset\AssetDefinition::getTable(), [
    ...
    if ($assetDefId > 0 && class_exists('Glpi\Asset\CustomFieldDefinition')) {
        ...
        $existingKm = $DB->request([
            'FROM'  => \Glpi\Asset\CustomFieldDefinition::getTable(),
            'WHERE' => [
                'assets_assetdefinitions_id' => $assetDefId,
        ...
        if (count($existingKm) == 0) {
            ...
            $DB->insert(\Glpi\Asset\CustomFieldDefinition::getTable(), [
                'assets_assetdefinitions_id' => $assetDefId,
                'system_name'   => 'km_inicial',
